...
StatisticsCollector::StatisticsCollector(Drop& drop, const Options& options)
  : m_drop(drop)
  , m_options(options)
  , m_nSent(0)
  , m_nReceived(0)
  , m_nNacked(0)
  , m_dropStartTime(time::steady_clock::now())
  , m_minRtt(std::numeric_limits<double>::max())
  , m_maxRtt(0.0)
  , m_sumRtt(0.0)
  , m_sumRttSquared(0.0)
{
  m_drop.afterData.connect(bind(&StatisticsCollector::recordData, this, _2));
  m_drop.afterNack.connect(bind(&StatisticsCollector::recordNack, this));
  m_drop.afterTimeout.connect(bind(&StatisticsCollector::recordTimeout, this));
}
...
StatisticsCollector::computeStatistics()
{
  ...
  statistics.dropStartTime = m_dropStartTime;
...
{
  os << "\n";
  os << "--- " << statistics.prefix << " drop statistics ---\n";
