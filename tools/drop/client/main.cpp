...
public:
  ...
  Runner(const Options& options)
    : m_drop(m_face, options)
    , m_statisticsCollector(m_drop, options)
    , m_tracer(m_drop, options)
...
main(int argc, char* argv[])
{
  ...
  visibleOptDesc.add_options()
    ...
    ("interval,i",  po::value<time::milliseconds::rep>()->default_value(getDefaultDropInterval().count()),
                    "drop interval, in milliseconds")
    ("timeout,o",   po::value<time::milliseconds::rep>()->default_value(getDefaultDropTimeoutThreshold().count()),
  ...
  std::cout << "DROP " << options.prefix << std::endl;
