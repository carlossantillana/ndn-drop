...
public:
  ...
  Runner(const Options& options)
    : m_options(options)
    , m_dropServer(m_face, m_keyChain, options)
    , m_tracer(m_dropServer, options)
    , m_signalSet(m_face.getIoService(), SIGINT)
  {
    m_dropServer.afterFinish.connect([this] {
      m_dropServer.stop();
      m_signalSet.cancel();
    });
  ...
  run()
  {
    std::cout << "DROP SERVER " << m_options.prefix << std::endl;
    ...
    std::cout << "\n--- " << m_options.prefix << " drop server statistics ---\n"
              << m_dropServer.getNDrops() << " packets processed" << std::endl;
    return 0;
  }
...
usage(std::ostream& os, const std::string& programName, const po::options_description& options)
{
  os << "Usage: " << programName << " [options] <prefix>\n"
     << "\n"
     << "Starts a NDN drop server that responds to Interests under name ndn:<prefix>/drop\n"
     << "\n"
     << options;
}
...
main(int argc, char* argv[])
{
  ...
  visibleDesc.add_options()
    ...
    ("freshness,f", po::value<time::milliseconds::rep>()->default_value(options.freshnessPeriod.count()),
                    "FreshnessPeriod of the drop response, in milliseconds")
    ("satisfy,p",   po::value(&nMaxDrops)->default_value(nMaxDrops),
                    "maximum number of drops to satisfy (0 = no limit)")
    ...
    ("quiet,q",     po::bool_switch(&options.wantQuiet),
                    "do not print a log message each time a drop packet is received")
  ...
  if (vm.count("version") > 0) {
    std::cout << "ndndropserver " << tools::VERSION << std::endl;
    return 0;
  }
  ...
  if (nMaxDrops < 0) {
    std::cerr << "ERROR: maximum number of drops to satisfy cannot be negative" << std::endl;
    return 2;
  }
  options.nMaxDrops = static_cast<size_t>(nMaxDrops);
...
main(int argc, char* argv[])
{
  return ndn::drop::server::main(argc, argv);
}
